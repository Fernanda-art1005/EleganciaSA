<div class="grid grid-cols-1 xl:grid-cols-3 gap-8">
                                ...
                                <td class="py-3.5 px-4 font-semibold text-gray-700 max-w-[220px] truncate" title="<?= htmlspecialchars($prod['nome']) ?>">
                                    <div class="flex items-center gap-2.5">
                                        <div class="w-8 h-8 rounded-lg overflow-hidden bg-gray-50 border border-sand-100 flex-shrink-0">
                                            <img src="<?= !empty($prod['imagem_url']) ? htmlspecialchars($prod['imagem_url']) : getProductImageUrl($prod['nome']) ?>" alt="<?= htmlspecialchars($prod['nome']) ?>" class="w-full h-full object-cover">
                                        </div>
    ...
    <div class="bg-white p-6 rounded-2xl border border-sand-100 shadow-sm h-fit">
        ...
        <form action="<?= $basePath ?>/estoque/salvar" method="POST" id="produto-form" class="space-y-4">
            ...
            <div>
                <label for="descricao" class="block text-[10px] font-bold uppercase tracking-wider text-gray-400 mb-1.5">Descrição Curta / Detalhes (Opcional)</label>
                <textarea id="descricao" name="descricao" placeholder="ex: Peça folheada a Ouro 18k, tamanho único, antialérgico." rows="2"
...
if (!empty($produtos)) {
    foreach ($produtos as $p) {
        $produtosMap[$p['id_produto']] = $p;
    }
}
?>
<script>
const produtosMap = <?= json_encode($produtosMap) ?>;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Auto-carrega para edição caso id_produto venha via URL (origem Dashboard)
    const urlParams = new URLSearchParams(window.location.search);
    const editId = urlParams.get('id_produto');
    if (editId && produtosMap[editId]) {
        setTimeout(() => {
            window.editarProduto(produtosMap[editId]);
